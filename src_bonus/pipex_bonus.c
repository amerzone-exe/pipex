...
/*   Updated: 2025/03/26 19:02:44 by jpiquet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	first_pipe(t_cmd *cmd, char *envp[], t_fd *fd, int i)
{
	...
	if (pid == 0)
	{
		close_more_files(fd->outfile, pipex[0], 0, 0);
		...
		close_more_files(fd->infile, pipex[1], 0, 0);
...
static void	middle_pipe(t_cmd *cmd, char *envp[], t_fd *fd, int i)
{
	int	pid;
	int	pipex[2];
	...
	if (pid == 0)
	{
		...
		if (dup2(fd->outpipe, STDIN_FILENO) == -1)
			exit_both_fd(pipex[0], pipex[1], cmd, fd->nb_cmd);
	...
	close_both_files(fd->outpipe, pipex[1]);
	fd->outpipe = pipex[0];
}
...
static void	last_pipe(t_cmd *cmd, char *envp[], t_fd *fd, int i)
{
	...
	if (pid_last == 0)
	{
		...
		close_more_files(fd->outpipe, pipex[0], 0, 0);
...
void	pipex(t_cmd *cmd, char *envp[], t_fd fd)
{
	...
	{
		if (i == 0)
			first_pipe(cmd, envp, &fd, i);
		else if (i == fd.nb_cmd - 1)
			last_pipe(cmd, envp, &fd, i);
		else
			middle_pipe(cmd, envp, &fd, i);
		i++;
	}
}
